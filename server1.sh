...
sed -i 's|export KEY_PROVINCE="CA"|export KEY_PROVINCE="Bulacan"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="Marilao"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="Iamjefjef23"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="jdbcmaster2@yahoo.com"|' /etc/openvpn/easy-rsa/vars
...
# download script
cd
wget https://raw.githubusercontent.com/jdbcmaster23/centos7/master/premium-script.tar.gz -O - -o /dev/null|sh
